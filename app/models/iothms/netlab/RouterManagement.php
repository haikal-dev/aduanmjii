...
{
	protected $ip = "172.16.124.29";
	protected $port = "8728";
	protected $user = "mkns";
	protected $pass = "1zptxA";
	...
	public function netlab_init(){
		if($this->ros->connect($this->ip, $this->port, $this->user, $this->pass)){
			
			$data = [];
			$devices = json_decode(file_get_contents("../app/views/netlab/devices.json"), true);
			
			$data['system'] 	= $this->ros->comm("/system/resource/print");
			$data['wireless']	= $this->ros->comm("/interface/wireless/registration-table/print");
			
			// get device for each MAC address
			$dhcp = $this->ros->comm("/ip/dhcp-server/lease/print");
			foreach($data['wireless'] as $k => $v){
				$s = explode("@", $v['signal-strength']);
				$data['wireless'][$k]['netlab_signal'] = (isset($s[0])) ? $s[0] : "";
				$data['wireless'][$k]['netlab_data_rate'] = (isset($s[1])) ? $s[1] : "";
				
				
				$data['wireless'][$k]['name'] = "New incoming device...";
				foreach($dhcp as $d){
					if($d['mac-address'] == $v['mac-address']){
						$data['wireless'][$k]['name'] = (isset($d['host-name'])) ? $d['host-name'] : "unnamed";
					}
				}
				
				$data['wireless'][$k]['owner'] = "";
				foreach($devices as $dev){
					if($dev['mac'] == $v['mac-address']){
						$data['wireless'][$k]['owner'] = strtoupper($dev['name']);
					}
				}
			}
